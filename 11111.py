class Object:
    def __init__(self, name, age):
        self.name = name
    def introduce(self):
        print(f'Я существо по имени {self.name}')
class Animal(Object):
...
# wolf = Wolf("Серый", 5, "млекопитающее", True)
# cow = Cow("Буренка", 4, "млекопитающее", "Зоопарк")
...
# cat.make_sound()
# cat.pet()
morning_roll_call = [Wolf("Серый", 5, "млекопитающее", True), Lion("Кинг", 7, "млекопитающее", True), Cow("Зорька", 4, "млекопитающее", "ФермерИван"), Cat("Мурзик", 2, "млекопитающее", "Семья Петровых"),Elephant("Джамбо", 12, "млекопитающее", False)]
def morning_check(animals_list):
    for animal in animals_list:
        animal.introduce()
        animal.make_sound()
        if isinstance(animal, DomesticAnimal):
            animal.pet()
morning_check(morning_roll_call)
visitors = [Visitor("Анна", 35, "посетитель", True), Child("Витя", 10, "посетитель", True), Visitor("Олег", 28, "посетитель", False)]
def excursion(visitors_list):
  return visitors
  for visitor in visitors_list:
    if has_ticket:
      visitor.look_at_animals()
      print(f'посетитель {self.name} смотрит на: Волк - {Wolf.make_sound()}')
    else:
      print(f'{self.name} не может смотреть на животных, нужно купить билет.')
excursion(visitors)